...
# Enhanced data storage for charts
...
def generate_balance_distribution_chart(balances):
    ...
    # Filter out zero and negative balances for pie chart
    positive_balances = {k: v for k, v in balances.items() if v > 0}
    if not positive_balances:
        # Create a default chart if no positive balances
...
def generate_mining_analysis_chart(chain_data):
    ...
    # Calculate mining times (simulated)
    ...
    p = np.poly1d(z)
    plt.plot(range(len(blocks)), p(range(len(blocks))), "r--", alpha=0.7)
...
def generate_network_activity_chart(network_data):
    ...
    activity_levels = [max(5, 20 + 10 * np.sin(i / 3)) for i in timestamps]  # Simulated activity

    plt.figure(figsize=(10, 6))
    plt.fill_between(timestamps, activity_levels, alpha=0.4, color='#3498db')
    plt.plot(timestamps, activity_levels, linewidth=2, color='#2980b9')
...
def generate_simblock_metrics_chart(analysis_data):
    """Generate bar chart for SimBlock metrics"""
...
def generate_pdf_report_route():
    """Enhanced PDF report with multiple charts"""
    try:
        script_dir = os.path.dirname(__file__)
